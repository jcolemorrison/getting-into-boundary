...
resource "aws_instance" "boundary_static_hosts_foo" {
  count = 2
  ...
  tags = {
    Name = "boundary-static-host-foo-${count.index}"
  }
}

resource "aws_instance" "boundary_static_hosts_bar" {
  count = 2
  ...
  tags = {
    Name = "boundary-static-host-bar-${count.index}"
  }
}
